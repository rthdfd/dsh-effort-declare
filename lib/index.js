 ...
 *
 * 零配置：装好后自动处理 llm-pi-ai 里所有「手填了 models 列表」的 provider，
 * 给缺声明的模型补上 reasoningEfforts，模型选择器里随即出现 Effort 菜单。
 *
 * 背景：dsh 的模型选择器只为「声明了 reasoningEfforts 的模型」显示 Effort 菜单。
 * 内置目录的模型自带声明；手填的第三方模型默认没有档位，请求路径还会按目录校验
 * （未声明的档位一律 UNSUPPORTED_REASONING_EFFORT）。DeepSeek 自家路由已自带
 * off / low / high / max——本插件把同一套档位补给第三方模型：
 *
 *   读取 settings 里的 llm-pi-ai 用户分节 → 给目标模型条目补上 reasoningEfforts
 *   （缺省才补，不覆盖已有）→ 经 settings 服务写回，live 生效。
 *
...
const DEFAULTS = {
  ...
  mode: 'apply',
  /**
   * 'auto'（默认）= 处理所有手填了 models 的 provider，零配置；
   * 也可以写成对象收窄到指定 provider：
   *   providers:
   *     my-gateway:
   *       models: '*'            # '*' | ['id1', 'id2'] | 'id'
...
export function apply(ctx, config) {
  ...
  const autoMode =
    ...
    rawProviders === null ||
    (isObject(rawProviders) && Object.keys(rawProviders).length === 0)
  const explicitProviders = autoMode ? {} : isObject(rawProviders) ? rawProviders : {}
  if (autoMode) log('零配置模式：将处理所有手填 models 的 provider')
    ...
    try {
      ...
      const targets = autoMode
        ? Object.entries(userProviders)
            .filter(([, route]) => isObject(route) && Array.isArray(route.models))
            .map(([providerName]) => [providerName, {}])
        : Object.entries(explicitProviders)
      if (targets.length === 0) {
        log('暂未发现可处理的 provider（配置出现后会自动重试）')
        scheduleRetry()
        return
      }
      ...
      const touched = []
      for (const [providerName, rawProviderCfg] of targets) {
        ...
        if (models === undefined) {
          if (!autoMode) log(`跳过「${providerName}」：用户分节里没有手填的 models 列表`)
          continue
        }
